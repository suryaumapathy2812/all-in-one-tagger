...
const executeScript = function (dirctory) {
    try {
        ...
        try {
            ...
            throw new Error('failed in the for loop')
        }


        core.debug("extractedCode \n ")
        core.debug(JSON.stringify(extractedCode, null, 2))
        ...
        // const allTopics = codeContent.map((code) => code.topics).flat();

        const topicCount = tags.reduce((accumulator, currentTopic) => {
            // Check if the topic already exists in the accumulator
            const existingTopic = accumulator.find(
                (topic) => topic.topic === currentTopic.topic,
            );

            // If the topic exists, update the count
            if (existingTopic) {
                existingTopic.count += currentTopic.count;
            } else {
                // If the topic does not exist, add it to the accumulator
                accumulator.push({
                    topic: currentTopic.topic,
                    count: currentTopic.count,
                });
            }
